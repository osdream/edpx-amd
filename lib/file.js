...
function searchModules(configFile, dir) {
    ...
    function scanDir(dir) {
        if (!fs.existsSync(dir)) {
            logger.error('No such file or directory: %s', dir);
        }
        else {
    ...
    for (var file in fileModuleMap) {
        if (fileModuleMap.hasOwnProperty(file)) {
            ...
            for (var i = 0; i < fileModules.length; i++) {
                ...
                if (mod.aliasIds) {
                    moduleIds = moduleIds.concat(mod.aliasIds);
                }
            }
            for (var j = 0; j < modules.length; j++) {
                mod = modules[j];
                if (moduleIds.indexOf(mod) !== -1) {
                    map[mod].push(file);
                }
            }
        }
 ...
 * @param {boolean=} options.resolveAll 是否处理package和paths
 * @param {string} options.outputType 输出类型：overwrite/copy/console/return
 * @return {?string}
 */
