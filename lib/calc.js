...
function alignify(array) {
    ...
    function tryIt() {
        ...
        for (var i = 0; i < array.length; i++) {
            var rowIndex = parseInt(i / possibleColumnNumber);
            lines[rowIndex] = lines[rowIndex] || [];
            lines[rowIndex].push(array[i].length + 2);
        }
        var firstLine = lines[0];
        colMaxArr = [];
        for (var i = 0; i < possibleColumnNumber; i++) {
            var col = [0];
    ...
    var lines = [];
    for (var i = 0; i < array.length; i++) {
        var rowIndex = parseInt(i / columnNumber);
 ...
 * 将依赖关系可视化，输出到console
 *
 * @param {Object} 依赖关系图数据
 * @param {string} 要展示依赖关系的模块
 * @return {string}
 */
